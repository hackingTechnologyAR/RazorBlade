...
def generate_report():
    ...
    with pd.ExcelWriter(excel_filename, engine='openpyxl') as writer:
        ...
        # Исправленный блок подбора ширины колонок
        for col_idx, col in enumerate(worksheet.columns, start=1):
            max_len = max(len(str(cell.value or '')) for cell in col)
            col_letter = get_column_letter(col_idx)
            worksheet.column_dimensions[col_letter].width = max(max_len + 3, 12)
            
    print(f"[+] Красивая Excel-таблица успешно создана: {excel_filename}")

    # 2. Построение графиков ответов
    plt.figure(figsize=(8, 5))
    status_counts = df['Response Status'].value_counts()
    
    colors = ['#2ecc71' if x == 'NOERROR' else '#e74c3c' if x == 'TIMEOUT' else '#3498db' for x in status_counts.index]
